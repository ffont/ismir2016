...
def calc_pulse_trains(lag, window, limit_pulse_trains):
    # This function has been modified from the original
    # Lag phases are tested from phase=0 to phase=lag (as described in paper)
    # pulse train multiples are computed as phase + b*v*period instead of phase - b*v*period
    if limit_pulse_trains:
        # Remove zeros at the end of window (if any)
        # This should only affect short sounds
        first_zero_pos = numpy.argmax(window[:][:][-1]==0)
        if window[first_zero_pos:].sum():
            # All zeros until the end
            window = window[:first_zero_pos]

    period = lag
    num_offsets = period
    samples = len(window)
    bp_mags = numpy.zeros( num_offsets )
    for phase in range(0, period, 1):
        mag = 0.0
        for b in range(4):
            ind = int(phase + b*period)
            # this is I_{ P, phi, 1)
            if ind >= 0 and ind < len(window):
                mag += window[ind]
            # this is I_{ P, phi, 2)
            # slow down by 2
    ...
    for i in xrange(num_frames):
        ...
        for j, cand in enumerate(cands):
            ...
            if not skip_calc_pulse_trains:
                mag, var = calc_pulse_trains(lag, overlapped[i], limit_pulse_trains=limit_pulse_trains)
            else:
                mag = 0.0
                var = 0.0
            tempo_scores[j] = mag
